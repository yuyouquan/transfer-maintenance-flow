...
const ANCHOR_SECTIONS = [
  { id: 'maintenance-spm-section-pipeline', label: '流水线', icon: <SyncOutlined /> },
  { id: 'maintenance-spm-section-info', label: '项目信息', icon: <FileTextOutlined /> },
  { id: 'maintenance-spm-section-team', label: '团队信息', icon: <TeamOutlined /> },
  { id: 'maintenance-spm-section-checklist', label: 'CheckList', icon: <CheckCircleOutlined /> },
  { id: 'maintenance-spm-section-review', label: '评审要素', icon: <AuditOutlined /> },
  { id: 'maintenance-spm-section-block', label: 'Block任务', icon: <StopOutlined /> },
  { id: 'maintenance-spm-section-legacy', label: '遗留任务', icon: <PushpinOutlined /> },
...
export default function MaintenanceSpmReviewPage({
  params,
}: {
  ...
  const reviewAccess = getMaintenanceSpmReviewAccess(application, currentUser.id);
  ...
  const handleApprove = () => {
    if (!canApprove) { message.warning('仅当前任务的维护SPM可在各角色审核通过后操作'); return; }
    ...
    addHistoryRecord({
      ...
      detail: `维护SPM审核通过，进入信息变更阶段${spmComment.trim() ? `；评审建议：${spmComment.trim()}` : ''}`,
    });
    message.success('维护SPM审核通过，流水线进入信息变更阶段');
    setApproveModalVisible(false);
    router.push(`/workbench/${id}`);
  };

  const handleReject = () => {
    if (!canReject) { message.warning('当前用户无权审核或审核阶段已变更'); return; }
    if (!spmComment.trim()) {
      message.warning('请填写维护SPM评审建议');
      return;
    }
    // 维护SPM 不通过即终态：整个转维流程失败并结束
    updateApplication(id, (app) => ({
      ...app,
      status: 'failed',
      failureReason: spmComment.trim(),
      pipeline: {
        ...app.pipeline,
        maintenanceSpmReview: 'failed',
      },
      updatedAt: new Date().toISOString(),
    }));
    addHistoryRecord({
      ...
      action: '维护SPM 关闭流水线',
      operator: currentUser.name,
      detail: `维护SPM审核不通过，转维流程已终止；驳回原因：${spmComment.trim()}`,
    });
    message.success('维护SPM审核不通过，转维流程已终止');
  ...
  const reviewRows = buildCloseReviewRows(application, [...checklistItems, ...reviewElements]);
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
      ...
      {/* 2. Project info */}
      <Card id="maintenance-spm-section-info" title="项目信息" style={{ marginBottom: 20 }}>
      ...
      <Card
        id="maintenance-spm-section-approval"
        title={
      ...
      >
        <p style={{ color: '#666' }}>审核负责人：{reviewer?.name ?? '未配置维护SPM'}</p>
        {!isReviewer && <Alert type="warning" showIcon title="仅当前任务的维护SPM可操作审核" style={{ marginBottom: 16 }} />}
        {/* Review status table */}
        ...
        {/* Maintenance SPM comment */}
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontWeight: 500, marginBottom: 8, color: '#333' }}>维护SPM评审建议</div>
          <TextArea
            rows={4}
            placeholder="请输入维护SPM评审建议（不通过时必填）"
            value={spmComment}
            onChange={(e) => setSpmComment(e.target.value)}
            maxLength={500}
            showCount
            disabled={!canOperate}
          />
        ...
        {isRejectionMode && (
          <Alert
            ...
            showIcon
            title="当前处于驳回处理模式"
            description="维护审核中有角色被拒绝，请核实情况后决定是否终止转维流程。维护SPM 不通过后流程将直接终止，申请人可在详情页发起重开。"
        ...
        {!canOperate && (
          <div style={{ textAlign: 'center', color: '#999', padding: '12px 0' }}>
            {application.pipeline.maintenanceSpmReview === 'success' && <Tag color="success">维护SPM审核已通过</Tag>}
            {application.pipeline.maintenanceSpmReview === 'failed' && <Tag color="error">维护SPM审核未通过</Tag>}
            {application.pipeline.maintenanceSpmReview === 'not_started' && <Tag color="default">维护SPM审核未开始</Tag>}
          </div>
        )}
      </Card>
      ...
      <Modal
        ...
        okText="确认通过"
        cancelText="取消"
        okButtonProps={{ disabled: !canApprove, style: { background: '#52c41a', borderColor: '#52c41a' } }}
      >
        <p>确认通过维护SPM审核？通过后流水线将进入「信息变更」阶段。</p>
      </Modal>
      ...
      >
        <p>确认不通过 维护SPM审核？<strong style={{ color: '#ff4d4f' }}>此操作将终止整个转维流程</strong>，申请状态变为「已失败」，不可继续审核。如需重新启动，申请人可在详情页点击「重新发起转维申请」。</p>
        {!spmComment.trim() && (
